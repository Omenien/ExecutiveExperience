...
//		}
    ...
    private void assemblePhysics()
    {
        ...
        for(IBaseItem item : items)
        {
            ...
            if(itemVO.meshId < 0 || bodyData == null)
            {
                continue;
            }
    ...
    private void recalculateSize()
    {
        ...
        for(int i = 0; i < items.size(); i++)
        {
            ...
            if(value.getScaleX() > 0 && value.getWidth() > 0)
            {
                ...
                if(upperX < value.getX() + value.getWidth())
                {
                    upperX = value.getX() + value.getWidth();
                }
            }
            else
            {
                ...
                if(lowerX > value.getX() + value.getWidth())
                {
                    lowerX = value.getX() + value.getWidth();
                }
            }
            if(value.getScaleY() > 0 && value.getHeight() > 0)
            {
                ...
                if(upperY < value.getY() + value.getHeight())
                {
                    upperY = value.getY() + value.getHeight();
                }
            }
            else
            {
                ...
                if(lowerY > value.getY() + value.getHeight())
                {
                    lowerY = value.getY() + value.getHeight();
                }
            }
    ...
    public void applyResolution(float mulX, float mulY)
    {
        ...
        if(getBody() != null)
        {
            essentials.world.destroyBody(getBody());
        }
        setBody(null);
    ...
    public void dispose()
    {
        if(essentials.world != null && body != null)
        {
            essentials.world.destroyBody(getBody());
        }
        setBody(null);
